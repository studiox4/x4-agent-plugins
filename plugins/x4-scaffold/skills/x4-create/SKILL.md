...
# /x4-create — Scaffold a New x4-mono Project
...
(You can always run /x4-env later to set this up)
```

If yes, delegate to the `/x4-env` skill in the new project directory.

If skip, inform the user:
```
Skipped env setup. Before running the project:
1. cd <project-name>
2. Run /x4-env to set up your database and auth
...
If `x4-project-tracker` and/or `x4-agent-team-ops` are installed, offer:
...
- Run /x4-add to add more apps later
```
